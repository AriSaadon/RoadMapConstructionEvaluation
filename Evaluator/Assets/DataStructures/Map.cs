...
public class Map
{
    ...
    /// <summary>
    /// Prunes the roads of the map that have not been used in any trajectory generation.
    /// </summary>
    public void PruneRoads()
    {
        List<Road> unusedRoads = new List<Road>();

        foreach (Road road in roads) //remove roads that have entirely been unused.
        {
        ...
        foreach (Road road in roads) //remove parts of the road that have not been used.
        {
            ...
            if (road.roadPoints.Count < 2) unusedRoads.Add(road);
        }
