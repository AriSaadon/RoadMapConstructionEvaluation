...
public class TrajGenerator
{
    ...
    /// </summary>
    /// <param name="map">The map we want to generate trajectories on, needs atleast two roads to work.</param>
    /// <param name="precisionError">The precision error we want the trajectories of our set to contain.</param>
    /// <param name="samplingDistance">The sampling distance/error we want the trajectories of our set to contain.</param>
    /// <param name="amount">The amount of random trajectories we want to generate.</param>
    /// <returns></returns>
    public List<List<(Vector3, int)>> GenerateTracks(Map map, int precisionError, int samplingDistance, int amount)
    {
        ...
        {
            allTrajectories.Add(GenerateTrack(map, precisionError, samplingDistance));
        }
    ...
    /// <param name="samplingDistance">The amount of sampling distance/error we want our trajectory to exhibit.</param>
    /// <returns>A list of location, time tuples.</returns>
    public List<(Vector3, int)> GenerateTrack(Map map, int precisionError, int samplingDistance)
    {
        Vector3[] path = null;
        ...
        float sampleDistance = samplingDistance;
        ...
        while (i != path.Length - 1)
        {
            ...
            else
            {
                ...
                sampleDistance = samplingDistance;
                elapsedTime += sampleDistance;
            }
        }
